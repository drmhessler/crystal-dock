...
namespace crystaldock
{
  ...
  DockPanel::DockPanel(MultiDockView *parent, MultiDockModel *model, int dockId)
      : QWidget(),
        ...
        aboutDialog_(QMessageBox::Information, "About Crystal Dock",
                     QString("<h3>Crystal Dock 2.7</h3>") + "<p>Copyright (C) 2024 Viet Dang (dangvd@gmail.com)" + "<p><a href=\"https://github.com/dangvd/crystal-dock\">https://github.com/dangvd/crystal-dock</a>" + "<p>License: GPLv3",
  ...
  void DockPanel::updateAnimation()
  {
    ...
    backgroundWidth_ = startBackgroundWidth_ + (endBackgroundWidth_ - startBackgroundWidth_) * currentAnimationStep_ / numAnimationSteps_;
    backgroundHeight_ = startBackgroundHeight_ + (endBackgroundHeight_ - startBackgroundHeight_) * currentAnimationStep_ / numAnimationSteps_;
  ...
  void DockPanel::drawGlass3D(QPainter &painter)
  {
    ...
    {
      int y = isTop()
                  ? isFloating() ? 2 * floatingMargin_ + 4 : 4
    ...
    int y = isBottom() ? height() - itemSpacing_ - k3DPanelThickness : height() - 125;
    if (isFloating())
    {
  ...
  bool DockPanel::addTask(const WindowInfo *task)
  {
    ...
    QPixmap taskIcon = appIcon.isNull() && !taskIconName.isEmpty()
                           ? loadIcon(taskIconName, kIconLoadSize)
                           : QPixmap();
  ...
  bool DockPanel::isValidTask(const WindowInfo *task)
  {
    ...
    if (!task->onAllDesktops && model_->currentDesktopTasksOnly() && task->desktop != WindowSystem::currentDesktop())
    {
      return false;
    }
  ...
  void DockPanel::updateLayout()
  {
    ...
    for (int i = 0; i < itemCount(); ++i)
    {
      ...
      if (isHorizontal())
      {
        ...
        if (is3D() && isBottom())
        {
          items_[i]->top_ -= k3DPanelThickness;
        }
  ...
  void DockPanel::updateLayout(int x, int y)
  {
    ...
    for (int i = 0; i < itemCount(); ++i)
    {
      ...
      if (i > 0)
      {
        if (isHorizontal())
        {
          items_[i]->left_ = items_[i - 1]->left_ + items_[i - 1]->getWidth() + itemSpacing_;
        }
        else
        { // Vertical
          items_[i]->top_ = items_[i - 1]->top_ + items_[i - 1]->getHeight() + itemSpacing_;
        }
      }
    }
    ...
    if (first_update_index == 0 && last_update_index < itemCount() - 1)
    {
      for (int i = last_update_index; i >= first_update_index; --i)
      {
        if (isHorizontal())
        {
          items_[i]->left_ = items_[i + 1]->left_ - items_[i]->getWidth() - itemSpacing_;
        }
        else
        { // Vertical
          items_[i]->top_ = items_[i + 1]->top_ - items_[i]->getHeight() - itemSpacing_;
        }
      }
    }
