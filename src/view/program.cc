...
namespace crystaldock
{
  ...
  void Program::draw(QPainter *painter) const
  {
    ...
    { // Show task count indicator.
      ...
      if (taskCount > kMaxVisibleTaskCount)
      {
         taskCount = kMaxVisibleTaskCount;
      }
      auto activeTask = getActiveTask();
      if (activeTask > kMaxVisibleTaskCount - 1)
      {
        activeTask = kMaxVisibleTaskCount - 1;
      }
      ...
      for (int i = 0; i < taskCount; ++i)
      {
        ...
        if (parent_->is3D())
        {
          const auto baseColor = useActiveColor
                                     ? model_->activeIndicatorColor()
                                     : model_->inactiveIndicatorColor();
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            if (activeTask >= 0)
            {
              // Cycles through tasks depending on CTRL Key.
              if (mod & Qt::ControlModifier)
              {
                auto nextTask = (activeTask  != 0) ? (activeTask - 1) : static_cast<int>(tasks_.size())-1; 
                WindowSystem::activateWindow(tasks_[nextTask].uuid);
              }
              else
              {
                auto nextTask = (activeTask < static_cast<int>(tasks_.size() - 1)) ? (activeTask + 1) : 0;
                WindowSystem::activateWindow(tasks_[nextTask].uuid);
              }
            }
            else
  ...
  QString Program::getLabel() const
  {
    ...
    return (taskCount > 1) ? label_ + " (" + (getActiveTask() >= 0 ? QString::number(getActiveTask() + 1) + "/" : "") + QString::number(tasks_.size()) + ")" : label_;
  }
